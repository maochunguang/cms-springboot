...
public class AppUtil  {
	...
	protected static MyLogger myLogger = MyLogger.getLogger(AppUtil.class);
	...
	public static boolean checkParam(String method, PageData pd){
		...
		if(falseCount>0){
			myLogger.error(method+"接口，请求协议中缺少 "+falseCount+"个 参数");
			for(int j=1;j<=falseCount;j++){
				myLogger.error("   第"+j+"个："+ tempArray[j-1]);
			}
		} else {
			result = true;
		}
	...
	public static List<PageData>  setListDistance(List<PageData> list, PageData pd){
		...
		try{
			...
			myLogger.error("缺失参数--user_longitude和user_longitude");
			myLogger.error("lost param：user_longitude and user_longitude");
		}
